...
export type ProblemLibraryDraft = {
  title: string
  description: string
...
export const EMPTY_PROBLEM_LIBRARY_DRAFT: ProblemLibraryDraft = {
  title: "",
  description: "",
  difficulty: "EASY",
  constraints: "",
      ...
      <TabsContent value="preview" className="mt-0">
        <div className="min-h-[9rem] rounded-2xl border border-slate-200 bg-slate-50/60 px-4 py-3">
          {value.trim() ? (
            ...
            <p className="text-sm text-slate-400">Chưa có nội dung để xem trước.</p>
          )}
        </div>
      </TabsContent>
    </Tabs>
...
export default function ProblemLibraryDraftForm({
  draft,
  isSubmitting,
  submitLabel,
  submittingLabel,
  ...
  draft: ProblemLibraryDraft
  isSubmitting: boolean
  submitLabel: string
  submittingLabel: string
  onChange: (patch: Partial<ProblemLibraryDraft>) => void
  onCancel: () => void
  onSave: () => void
}) {
  return (
    <div className="space-y-5">
      <div className="grid gap-4 md:grid-cols-2">
        <FieldBlock label="Tiêu đề bài tập">
          <Input
            placeholder="Ví dụ: Mảng hai số có tổng bằng target"
            value={draft.title}
        ...
        <FieldBlock label="Độ khó">
          <select
            ...
            onChange={(event) =>
              onChange({ difficulty: event.target.value as ProblemLibraryDraft["difficulty"] })
            }
      ...
      <FieldBlock label="Mô tả bài toán">
        <MarkdownEditorField
          rows={7}
          placeholder="Nhập đề bài cho problem bank..."
          value={draft.description}
          onChange={(value) => onChange({ description: value })}
        />
      </FieldBlock>

      <FieldBlock label="Ràng buộc">
        <MarkdownEditorField
          rows={6}
      ...
      <div className="space-y-3">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-500">
            Mã khởi tạo mặc định cho bài luyện tập C++ trong problem bank.
          </p>
        </div>
      ...
      <div className="space-y-3">
        <div>
          ...
          <p className="mt-1 text-xs text-slate-500">
            Khai báo input và output mong đợi cho hệ thống chấm bài của problem bank.
          </p>
        </div>
        <TestCaseManager
